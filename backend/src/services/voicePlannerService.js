...
import { getDataRoot } from '../config/database.js';
...
const TOKEN_FILE = path.join(getDataRoot(), 'voice-planner-google-token.json');
const STATE_FILE = path.join(getDataRoot(), 'voice-planner-oauth-state.txt');
...
export function getVoicePlannerStatus() {
...
export async function processVoicePlannerCommand({ audioBase64, mimeType, text, timezone = DEFAULT_TIMEZONE }) {
  ...
  const normalized = normalizeParsedCommand(parsed, transcript, timezone);

  if (normalized.type === 'calendar') {
    const googleResult = await createCalendarEvent(normalized, transcript, timezone);
    return { transcript, parsed: normalized, googleResult };
  }

  if (normalized.type === 'task') {
    const googleResult = await createTask(normalized, transcript);
    return { transcript, parsed: normalized, googleResult };
  }
...
async function getGoogleAccessToken() {
  ...
  if (!token.refresh_token) throw new Error('Google token expired and no refresh token is available. Reconnect Google.');
...
async function transcribeAudio({ audioBase64, mimeType = 'audio/webm' }) {
  ...
  if (!openaiKey) throw new Error('OPENAI_API_KEY is not configured for transcription.');
...
async function createCalendarEvent(command, transcript, timezone) {
  ...
  const response = await fetch('https://www.googleapis.com/calendar/v3/calendars/primary/events', {
    ...
    body: JSON.stringify({
      ...
      description: buildDescription(command.notes, transcript),
...
function buildDescription(notes, transcript) {
  return [notes, `תמלול: ${transcript}`].filter(Boolean).join('\n\n');
}
