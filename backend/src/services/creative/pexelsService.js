...
/**
 * @param {string} query
 * @param {{ perPage?: number, page?: number, orientation?: 'landscape'|'portrait'|'square' }} [opts]
...
export async function searchVideoUrls(query, opts = {}) {
  ...
  const page = Math.min(80, Math.max(1, parseInt(String(opts.page || 1), 10) || 1));
  const orientation = opts.orientation || 'portrait';
  const q = encodeURIComponent(query.trim().slice(0, 200));
  const url = `${PEXELS_ROOT}/search?query=${q}&per_page=${perPage}&page=${page}&orientation=${orientation}`;
