...
router.get('/track/click/:dealId', (req, res) => {
  try {
    ...
    // Redirect — prefer listing /itm/ when DB still has SERP placeholders
    res.redirect(resolveDealOutboundUrl(deal));
  } catch (error) {
    console.error('Click tracking error:', error);
    const dealFallback = prepare('SELECT * FROM deals WHERE id = ?').get(req.params.dealId);
    if (dealFallback) {
      const target = resolveDealOutboundUrl(dealFallback);
      res.redirect(target);
    } else {
...
// Seed sample deals (prefers live eBay listings when API is configured)
router.post('/seed-deals', authenticateToken, requireRole('admin'), async (req, res) => {
  try {
    const maxRows = Number(req.body?.maxRows) > 0 ? Math.min(Number(req.body.maxRows), 500) : 120;
    const result = await adminReseedSampleDeals({
      prepare,
      getDb,
      saveDatabase,
      maxRows
    ...
    res.json({
      message: `Seed complete (${result.mode}).`,
      ...result
    });
  } catch (error) {
