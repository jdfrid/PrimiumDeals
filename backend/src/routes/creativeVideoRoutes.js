...
router.get('/settings', authenticateToken, requireRole('admin', 'editor'), (req, res) => {
  try {
...
router.put('/settings', authenticateToken, requireRole('admin', 'editor'), (req, res) => {
  try {
    const body = req.body || {};
    ...
    console.error('Creative studio settings:', e);
    res.status(500).json({ error: e.message });
  }
});

router.get('/options', authenticateToken, requireRole('admin', 'editor'), (req, res) => {
  try {
    res.json({
...
router.get('/jobs', authenticateToken, requireRole('admin', 'editor'), (req, res) => {
  try {
...
router.get('/jobs/:id', authenticateToken, requireRole('admin', 'editor'), (req, res) => {
  try {
...
router.post('/jobs', authenticateToken, requireRole('admin', 'editor'), async (req, res) => {
  try {
...
router.post('/jobs/:id/retry', authenticateToken, requireRole('admin', 'editor'), async (req, res) => {
  try {
    const id = parseInt(req.params.id, 10);
